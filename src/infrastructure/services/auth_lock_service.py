...
class AuthLockService:
    ...
    async def increment_failed_attempts(
        ...
        logger.debug("User %s now has %s failed attempts.", user_email, current_attempts)

        if current_attempts == 1:
            logger.debug("Setting expiry for user %s failed attempts key to %s minutes", user_email, ACCOUNT_LOCKOUT_DURATION_MINUTES)
            await self.redis_client.expire(
                key, timedelta(minutes=ACCOUNT_LOCKOUT_DURATION_MINUTES)
            )

        if current_attempts >= MAX_FAILED_OTP_ATTEMPTS:
            logger.info("Account locked for user %s due to exceeding max failed attempts (%s)", user_email, MAX_FAILED_OTP_ATTEMPTS)
    ...
    async def get_failed_attempts(self, user_email: str) -> Tuple[int | None, Error]:
        ...
        if err:
            logger.error("Error retrieving failed attempts for user %s: %s", user_email, err.message)
            return None, err
        logger.debug("User %s has %s failed attempts.", user_email, attempts if attempts else 0)
    ...
    async def lock_account(
        ...
        logger.debug("Locking account for user %s for %s minutes", user_email, duration_minutes if duration_minutes else ACCOUNT_LOCKOUT_DURATION_MINUTES)
        ...
        logger.info("Account %s locked until: %s", user_email, datetime.utcnow() + timedelta(minutes=lock_duration))
        return None
        ...
        if lock_data_raw:
            try:
                ...
                return True, FailedAttemptError(
                    "Account is locked. Try again after %s minutes." % ACCOUNT_LOCKOUT_DURATION_MINUTES
                )
            except json.JSONDecodeError:
                logger.warning("Could not decode lock data for user %s. Assuming not locked.", user_email)
