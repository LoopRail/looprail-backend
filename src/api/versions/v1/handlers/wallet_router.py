...
async def process_withraw_request(
    ...
    rq_manager.get_queue().enqueue(
        ...
        wallet_name=config.block_rader.wallets.wallets[
            0
        ].wallet_id,  # Assuming default wallet for now
        ledger_id=config.ledger.ledgers.ledgers[
            0
        ].ledger_id,  # Assuming default ledger for now
    )
