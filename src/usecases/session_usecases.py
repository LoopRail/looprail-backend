...
class SessionUseCase:
    def __init__(
        ...
        logger.debug("SessionUseCase initialized with refresh token expiry: %s days", refresh_token_expires_in_days)
    ...
    async def create_session(
        ...
        logger.debug("Creating session for user %s on platform %s, device %s from IP %s", user_id, platform, device_id, ip_address)
        ...
        if err:
            logger.error("Failed to create session in repository for user %s: %s", user_id, err.message)
            return None, "", err
        ...
        if err:
            logger.error("Failed to create refresh token for session %s: %s", session.id, err.message)
            return None, "", err
        logger.info("Session %s and refresh token created for user %s", session.id, user_id)
    ...
    async def revoke_session(self, session_id: SessionId) -> Error:
        ...
        if err:
            logger.error("Failed to revoke session %s in repository: %s", session_id, err.message)
            return err
        logger.debug("Session %s revoked in repository. Revoking associated refresh tokens.", session_id)
        err = await self.refresh_token_repository.revoke_refresh_tokens_for_session(
            session_id
        )
        if err:
            logger.error("Failed to revoke refresh tokens for session %s: %s", session_id, err.message)
            return err
    ...
    async def rotate_refresh_token(
        ...
        logger.debug("Rotating refresh token for session %s", old_refresh_token.session_id)
        ...
        if err:
            logger.error("Failed to mark old refresh token %s as replaced: %s", old_refresh_token.id, err.message)
            return None, err
        ...
        if err:
            logger.error("Failed to create new refresh token for session %s: %s", old_refresh_token.session_id, err.message)
            return None, err
        logger.info("Refresh token rotated successfully for session %s", old_refresh_token.session_id)
    ...
    async def revoke_all_user_sessions(self, user_id: UserId) -> Error:
        ...
        if err:
            logger.error("Failed to revoke all sessions for user %s in repository: %s", user_id, err.message)
            return err
        logger.debug("All sessions for user %s revoked in repository. Revoking associated refresh tokens.", user_id)
        ...
        for session in sessions:
            ...
            if err:
                logger.error("Failed to revoke refresh tokens for session %s (user %s): %s", session.id, user_id, err.message)
                return err  # or log and continue
        logger.info("All sessions and associated refresh tokens revoked for user %s", user_id)
        return None
    ...
    async def get_valid_refresh_token_by_hash(
        ...
        refresh_token, err = await self.refresh_token_repository.get_valid_refresh_token_by_hash(
            refresh_token_hash
        )
        if err:
