...
def make_withdrawal_request(currency=Currency.NAIRA, bank_code="070", account_number="0123456789", account_name="John Doe", bank_name="Fidelity Bank"):
    req = MagicMock()
...
class TestInstitutionCodeResolution:
    ...
    def _build_usecase(self, banks_data: BanksData, country_code: str | None):
        ...
        usecase.service.paycrest_service.create_payment_order = AsyncMock(
            return_value=(MagicMock(data=MagicMock(payment_id="p1", receive_address="0xABC")), None)
        )
        usecase.service.transaction_usecase.update_transaction_status = AsyncMock(return_value=None)
    ...
    async def test_valid_bank_id_resolves_to_swift_code(self):
        ...
        with patch(
            ...
            err = await WalletManagerUsecase._execute_bank_transfer_withdrawal(
                usecase, user=MagicMock(), withdrawal_request=withdrawal_request, transaction=transaction
            )
    ...
    async def test_missing_country_code_returns_error(self):
        ...
        banks = make_banks_data({"NG": [{"name": "Fidelity Bank", "id": "070", "code": "FIDTNGLA"}]})
