...
async def test_create_challenge_success(client, mock_security_usecase):
    ...
    response = client.post("/api/v1/auth/challenge", json={"code_challenge": "cc_test"})
...
@pytest.mark.asyncio
async def test_passcode_login_success(
    client, mock_session_usecase, mock_security_usecase, mock_jwt_usecase
):
    ...
    user_public_data = UserPublic.model_validate(mock_user).model_dump(
        exclude_none=True
    )
    mock_user_usecase.load_public_user.return_value = (user_public_data, None)
    from src.main import app
